...
function Cli (opts) {
  var standard = require('../lib/linter.js')(opts)
  ...
  try {
    ...
    var packageOpts = require(path.join(root, 'package.json'))
    if (packageOpts) {
      opts.cmd = Object.keys(packageOpts.bin)[0]
      if (!opts.homepage) opts.homepage = packageOpts.homepage
      if (!opts.bugs) opts.bugs = packageOpts.bugs ? packageOpts.bugs.url : null
    }
  } catch (e) {}
